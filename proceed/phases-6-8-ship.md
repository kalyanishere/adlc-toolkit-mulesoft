...
- `/wrapup` will: update spec/task statuses, append assumptions/lessons, run any MuleSoft gates (Policies.md presence when API artifacts changed, governance scan, mule-preflight), generate the ship summary, deploy via `/canary` (`mvn deploy -P<env>` + DX MCP `deploy_mule_application`/`update_mule_application`; API Manager policy promotion via Platform MCP `apply_policy_to_instance`), and (Step 4b) commit the ADLC artifact changes onto `main` in the primary repo as a separate `chore(REQ-xxx): capture knowledge ...` commit.

**Step 3 — Verify `/wrapup` succeeded** before proceeding to cleanup. If it surfaced a Policies.md blocker, governance scan failure, mule-preflight failure, or a deploy failure, STOP — do NOT remove the worktree. The user resolves the blocker and re-runs `/wrapup` (or `/canary` directly). Note: state finalization (Step 1) has already happened, so the dashboard correctly shows the REQ as merged regardless. The wrapup blocker is a **deploy blocker**, not a **merge blocker**.
